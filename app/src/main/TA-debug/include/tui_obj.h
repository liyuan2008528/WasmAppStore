...
extern int32_t cal_dp2px_factor();

EXPORTED void set_screen_label(const TEE_TUIScreenLabel *new_label);
...
EXPORTED int32_t cal_dp2px_factor();
/* -------------------------------------------- */
#define COLOR_PREFIX       '#'
#define P_SUFFIX           'p'
#define D_SUFFIX           'd'
...
#define DP_MASK            (0x10000000)
#define RATIO_MASK         (PERCENT_MASK | PERMIL_MASK)
#define TO_ABS(x)          ( (x) & (~RATIO_MASK)   )
#define DP_TO_ABS(x)       ( (x) & (~DP_MASK)      )
#define TO_PERCENT(x)      ((x == 0) ? x :( (x) |   PERCENT_MASK  ))
#define TO_PERMIL(x)       ((x == 0) ? x :( (x) |   PERMIL_MASK   ))
#define TO_DP(x)            ((x == 0) ? x :( (x) |  DP_MASK   ))
#define IS_PERCENT(x)      ( (x) &   PERCENT_MASK  )
#define IS_PERMIL(x)       ( (x) &   PERMIL_MASK   )
#define IS_DP(x)           ( (x) &   DP_MASK       )
#define UN_RATIO(v,base)     \
    do { if(IS_PERCENT(v)) \
        ...
        (v) = (DP_TO_ABS(v))*(cal_dp2px_factor()); \
    } while(0)

#define UN_RATIO_NEW(v,base)     \
    do { if(IS_PERCENT(v)) \
        (v) = (base)*(TO_ABS(v))/100; \
        else if(IS_PERMIL(v)) \
        (v) = (base)*(TO_ABS(v))/1000; \
...
enum draw_shape
{ DRAW_RECT = (1 << 0), DRAW_ROUND_RECT = (1 <<1 ), DRAW_LINE = (1<< 2) };
...
struct win_info {
	...
	int32_t screen_width_real;
	int32_t screen_height_real;
	int32_t bits_per_pixel;
	int32_t line_length;
	int32_t smem_length;
	int32_t font_type;
	uint32_t  flag1; /* 0 can draw, 1 is showing, cann't draw */
	char *frame_buffer1;
	char *frame_buffer2;
	char *draw_buff;
	char *show_buff;
...
struct ui_text_t {
	...
	uint32_t text_color;            /* for bg in pure color: default to BLACK=0 */
};
...
struct ui_obj_t {
	...
	uint32_t layout_marginLeft;              /* abs value: 0 for most cases if ignored */
	uint32_t layout_marginRight;              /* abs value: 0 for most cases if ignored */
	uint32_t layout_marginTop;              /* abs value: 0 for most cases if ignored */
	uint32_t layout_marginBottom;              /* abs value: 0 for most cases if ignored */
	uint32_t paddingLeft;             /* abs value: 0 for most cases if ignored */
	uint32_t paddingRight;             /* abs value: 0 for most cases if ignored */
	uint32_t paddingTop;             /* abs value: 0 for most cases if ignored */
	uint32_t paddingBottom;             /* abs value: 0 for most cases if ignored */
	uint32_t layout_gravity;             /* abs value: 0 for most cases if ignored */
	...
	uint32_t on_top;    /* runtime boolean: if NO, no need to show it. default to YES. */
...
struct ui_label_t {
	...
	uint32_t r_flag;              /*  for round rect */
	uint32_t img_gravity;          /* add for img  */
};
...
struct ui_edittext_t
{
	...
	uint32_t maxlen;              /* */
	uint32_t hint_text_color;            /* for bg in pure color: default to BLACK=0 */
	...
	int32_t old_len;
	int32_t edit_shape;
	char *hint_text;                /* for soft_kbd, it points to short_text. */
	bool focus;
	bool cursor_flag;
	bool cursor_status;
	bool is_entry_set;
...
EXPORTED int32_t is_in_security_indicator(const struct ui_label_t *si, int32_t x, int32_t y);
...
EXPORTED void ui_draw_frame(const struct ui_frame_t *frame);
EXPORTED void ui_draw_box(const struct ui_box_t *box);
...
int font_init();
int font_close();
int get_font_size_offset();
int event_enable();/* for event sync */
int event_disable();
/* -------------------------------------------- */
#endif
