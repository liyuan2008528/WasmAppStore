 ...
 *
 * Copyright(C), 2013-2018, Huawei Tech. Co., Ltd. ALL RIGHTS RESERVED. \n
 *
...
enum __TEE_Operation_Constants {
	...
	TEE_OPERATION_AE = 4,                   /**< 认证加密 */
	TEE_OPERATION_DIGEST = 5,               /**< 摘要 */
	TEE_OPERATION_ASYMMETRIC_CIPHER = 6,    /**< 非对称加解密 */
	TEE_OPERATION_ASYMMETRIC_SIGNATURE = 7, /**< 非对称签名 */
	TEE_OPERATION_KEY_DERIVATION =  8 ,   /**< 密钥派生 */
	TEE_OPERATION_ASYMMETRIC_VERIFY = 9     /**< 非对称认证 */
};
#endif
 ...
 * 注意:对称算法中的nopad模式输入长度要求16字节(AES)和8字节(DES/3DES)对齐，需要TA(Trusted Application，可信应用)来做填充
*/
enum __TEE_CRYPTO_ALGORITHM_ID {
...
#ifdef GP_SUPPORT
	TEE_ALG_RSAES_PKCS1_OAEP_MGF1_SHA224 = 0x60310230,  /**< RSAES_PKCS1_OAEP_MGF1_SHA224 */
	TEE_ALG_RSAES_PKCS1_OAEP_MGF1_SHA256 = 0x60410230,  /**< RSAES_PKCS1_OAEP_MGF1_SHA256 */
	TEE_ALG_RSAES_PKCS1_OAEP_MGF1_SHA384 = 0x60510230,  /**< RSAES_PKCS1_OAEP_MGF1_SHA384 */
	TEE_ALG_RSAES_PKCS1_OAEP_MGF1_SHA512 = 0x60610230,  /**< RSAES_PKCS1_OAEP_MGF1_SHA512 */
#else
	...
	TEE_ALG_ECDSA_SHA1 = 0x70001042,                    /**< ECC SHA1*/
	TEE_ALG_ECDSA_SHA224 = 0x70002042,                  /**< ECC SHA224*/
	TEE_ALG_ECDSA_SHA256 = 0x70003042,                  /**< ECC SHA256*/
	TEE_ALG_ECDSA_SHA384 = 0x70004042,                  /**< ECC SHA381*/
	TEE_ALG_ECDSA_SHA512 = 0x70005042,                  /**< ECC SHA512*/
	TEE_ALG_ECDH_DERIVE_SHARED_SECRET = 0x80000042,     /**< ECDH SHARED SECRET*/
    ...
    TEE_ALG_SM2_DSA_SM3 = 0x70006045,                  /**< SM2 DSA SM3 */
    TEE_ALG_SM2_KEP     = 0x60000045,                  /**< SM2 KEP */
    TEE_ALG_SM2_PKE     = 0x80000045,                  /**< SM2 PKE */
...
#ifdef GP_SUPPORT
struct block_mode_helper {
    uint8_t partial_block[16];
	uint8_t tmp[16];
	uint8_t tmp2[16];
	uint8_t prev_iv[16];
	uint8_t prev_prev_iv[16];
	uint32_t pos;
	uint32_t doFinal;
	uint32_t total_nmb;
};
...
struct __TEE_OperationHandle {
    ...
    struct block_mode_helper blocks;
#endif
 ...
 *
 * @par 描述:
 * 加解密初始化，包括AES和DES初始化，不推荐DES，推荐AES-CBC-256
 *
 * @attention 无
 *
 * @param operation [IN/OUT]  模块句柄 #TEE_OperationHandle
 ...
 * @param srcLen    [IN]      输入加密/解密数据长度 NOPAD模式下AES数据长度为16字节倍数，DES数据长度为8字节倍数
 ...
 *
 * @par 描述:
 * 摘要运算，支持SHA1，SHA224，SHA256，SHA384，SHA512，推荐SHA256
 *
 * @attention 
 * 不支持MD5
 *
 ...
 *
 * @par 描述:
 * 摘要运算结束，支持SHA1，SHA224，SHA256，SHA384，SHA512，推荐SHA256
 *
 * @attention 
 * 不支持MD5
 *
...
/**
 * @ingroup  crypto
 * @brief 认证解密运算结束，并计算tag和输入tag进行比较
 *
 * @par 描述:
 * 认证解密运算结束，并计算tag和输入tag进行比较，只支持AES-CCM
 *
 * @attention 
 * 不支持AES-GCM
 *
